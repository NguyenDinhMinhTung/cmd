    ...
    class KeyboardHook
    {
        ...
        private IntPtr KeyboardHookProc(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0)
            {
                ...
                {
                    if (KeyDown != null)
                        KeyDown(null, new KeyEventArgs(Keyboard.PrimaryDevice, new HwndSource(0, 0, 0, 0, 0, "", IntPtr.Zero), 0,KeyInterop.KeyFromVirtualKey(kbStruct.VirtualKeyCode)));
                }
                else if (wParam == (IntPtr)WM_KEYUP)
                {
                    if (KeyUp != null)
                        KeyUp(null, new KeyEventArgs(null,null,0,(Key)kbStruct.VirtualKeyCode));
                }
            }
